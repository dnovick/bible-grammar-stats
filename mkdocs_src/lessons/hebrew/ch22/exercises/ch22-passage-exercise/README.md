# "Qal Participle in Context" — Ch22 Passage Exercise

## Description

This passage exercise presents 15 highlighted participle forms drawn from eleven OT passages spanning Genesis, Exodus, Leviticus, Numbers, and Deuteronomy. Passages were selected to illustrate the full range of participle functions: predicate passive participles in the blessing and curse formulas (Gen 3:14, 4:11, 14:19, 24:27), predicate and circumstantial active participles (Gen 18:1, Exo 3:2, Num 14:14), the substantive participle used as a title (Gen 4:9, Lev 25:25), occupational/progressive active participles (Gen 37:2, Num 27:17), and attributive passive participles (Exo 6:6, Deu 9:10, Lev 2:4–5). Students identify active/passive, root, root class, gender/number, syntactic function, and gloss for each highlighted form.

### Conjugation coverage

| Function Type | Count | Forms |
|---|---|---|
| Passive predicate (blessing/curse) | 4 | אָר֤וּר (×2), בָּר֤וּךְ (×2) |
| Active predicate / circumstantial | 4 | יֹשֵׁ֤ב, בֹּעֵ֣ר, עֹמֵ֣ד, רֹעֶ֩ה (Gen 37:2) |
| Active substantive (noun/title) | 4 | שֹׁמֵ֥ר, גֹאֲל֔וֹ, רֹעֶ֖ה (Num 27:17), יֹשְׁבֵ֣י |
| Passive attributive | 3 | נְטוּיָ֖ה, כְּתֻבִ֣ים, בְּלוּלֹ֥ת |

## Files

| File | Use |
|---|---|
| [ch22-passage-exercise.md](ch22-passage-exercise.md) | Reference copy — static answer key at bottom |
| [ch22-passage-exercise.html](ch22-passage-exercise.html) | Classroom use — fillable fields, per-form ▶ Answer reveal, Show/Hide/Clear All controls |
| [ch22-passage-exercise.pdf](ch22-passage-exercise.pdf) | Print or tablet use — AcroForm text fields, answer rows always visible |
